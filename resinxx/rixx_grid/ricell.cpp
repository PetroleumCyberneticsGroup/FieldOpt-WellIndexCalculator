...
bool RICell::isLongPyramidCell(double maxHeightFactor,
  ...
  for ( face = 0; face < 6 ; ++face) {
    ...
    {
      return true;
      // Collapse of a complete face is detected.
      // This is possibly the top of a pyramid

      // "face" has the index to the collapsed face.
      // We need the size of the opposite face to
      // compare it with the pyramid "roof" length.
      ...
      // Both ratios have to be above threshold to detect
      // a pyramid-ish cell Only test this if we have all
      // nonzero edge lenghts.

    else if  (zeroLengthEdgeCount == 0) {
      // If the four first faces are ok, the two last must be as well
...
// -----------------------------------------------------------------
bool RICell::isCollapsedCell(double nodeNearTolerance) const {
  const std::vector<cvf::Vec3d>& nodes = m_hostGrid->mainGrid()->nodes();
...
// -----------------------------------------------------------------
// Returns an area vector for the cell face. The direction is the
// face normal, and the length is equal to the face area (projected
// to the plane represented by the diagonal in case of warp). The
// components of this area vector are equal to the area of the face
...
// -----------------------------------------------------------------
// Find the intersection between the cell and the ray. The point
// closest to the ray origin is returned in \a intersectionPoint,
// while the return value is the total number of intersections with
// the 24 triangles the cell is interpreted as. If no intersection
// is found, the intersection point is untouched.
int RICell::firstIntersectionPoint(const cvf::Ray& ray,
...
// -----------------------------------------------------------------
void RICell::faceIndices(cvf::StructGridInterface::FaceType face,
                          caf::SizeTArray4* indices) const {
