...
int cvf::RigHexIntersectionTools::lineHexCellIntersection(
  ...
  for ( int face = 0; face < 6 ; ++face )
  {
    ...
    cvf::Vec3d faceCenter = cvf::GeometryTools::computeFaceCenter(
        hexCorners[faceVertexIndices[0]],
    ...
    for ( int i = 0; i < 4; ++i ) {
      ...
      int intsStatus = cvf::GeometryTools::intersectLineSegmentTriangle(
          p1, p2,
                                       ...
                                       const cvf::Vec3d hexCorners[8])
{
  ...
  for ( int face = 0; face < 6; ++face )
  {
    ...
    {
      int next = i < 3 ? i + 1 : 0;
      if ( ray.triangleIntersect(hexCorners[faceVertexIndices[i]],
                                 hexCorners[faceVertexIndices[next]],
                                 faceCenter) )
      {
        ++intersections;
  ...
  for (int face = 0; face < 6; ++face)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      bool isIntersectingPlane =
          caf::HexGridIntersectionTools::planeTriangleIntersection(
              fracturePlane,
      ...
      if (isIntersectingPlane)
      {
        ...
        intersectionLineSegments.push_back(
            {triangleIntersectionPoint1.vx,
             triangleIntersectionPoint2.vx });
      }
    }
...
// -----------------------------------------------------------------
bool cvf::RigHexIntersectionTools::planeHexIntersectionPolygons(
  ...
  fracturePlane.setFromPointAndNormal(
      transformMatrixForPlane.translation(),
  ...
  isCellIntersected =
      planeHexCellIntersection(hexCorners.data(),
                               fracturePlane,
                               intersectionLineSegments);

  RigCellGeometryTools::createPolygonFromLineSegments(intersectionLineSegments,
                                                      polygons);
...
bool operator<(const cvf::HexIntersectionInfo& hi1,
  ...
  if ( hi1.m_hexIndex != hi2.m_hexIndex ) {
    return hi1.m_hexIndex < hi2.m_hexIndex;
  }

  if ( hi1.m_face != hi2.m_face ) {
    return hi1.m_face < hi2.m_face;
  }
  ...
  if ( cvf::Math::abs(
      hi2.m_intersectionPoint.x() - hi1.m_intersectionPoint.x()) > tolerance
      ) {
    return hi1.m_intersectionPoint.x() < hi2.m_intersectionPoint.x();
  }

  if ( cvf::Math::abs(
      hi2.m_intersectionPoint.y() - hi1.m_intersectionPoint.y()) > tolerance
      ) {
    return hi1.m_intersectionPoint.y() < hi2.m_intersectionPoint.y();
  }

  if ( cvf::Math::abs(
      hi2.m_intersectionPoint.z() - hi1.m_intersectionPoint.z()) > tolerance
      ) {
