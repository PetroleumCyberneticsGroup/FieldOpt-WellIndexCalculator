...
//  explicit String(int64 number);
  ...
  // Useful when you need a const char* pointer.
  CharArray toAscii() const;
...
//  int64 toInt64(int64 defaultValue) const;
  ...
  //----------------------------------------------------------------
  // Same as std::string::npos. This value, when used as the value
  // for a count parameter n in string's member functions, roughly
  // indicates "as many as possible".
