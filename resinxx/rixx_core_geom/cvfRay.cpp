...
//------------------------------------------------------------------
bool Ray::planeIntersect(const Plane& plane,
                         Vec3d* intersectionPoint) const
{
  ...
  // We want the plane normal facing the ray so flip direction of
  // the plane
  if (vd > 0)
  {
    pn *= -1;
    ...
    if (t >= 0)
    {
      ...
      if (m_distanceLimitedRay) {
        double distanceSquared =
            origin().pointDistanceSquared(m_origin + t*m_direction);

        if (m_minDistanceSquared < cvf::UNDEFINED_DOUBLE_THRESHOLD
            && distanceSquared < m_minDistanceSquared) {
          return false;
        }

        if (m_maxDistanceSquared < cvf::UNDEFINED_DOUBLE_THRESHOLD
            && distanceSquared > m_maxDistanceSquared) {
          return false;
        }
      }
...
String Ray::debugString() const {
  ...
  String str = "Ray: ";
  str += " origin: x=" +
      String::number(m_origin.x()) + " y=" +
      String::number(m_origin.y()) + " z=" +
      String::number(m_origin.z());

  str += " direction: x=" +
      String::number(m_direction.x()) + " y=" +
      String::number(m_direction.y()) + " z=" +
      String::number(m_direction.z());
...
void Ray::setMinimumDistance(double distance)
{
  ...
  m_distanceLimitedRay =
      (m_minDistanceSquared < cvf::UNDEFINED_DOUBLE_THRESHOLD)
          || (m_maxDistanceSquared < cvf::UNDEFINED_DOUBLE_THRESHOLD);
}
...
//
// Useful for limiting the ray when picking on models with clipping
// planes
