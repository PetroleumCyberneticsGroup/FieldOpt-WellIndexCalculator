...
void caf::AppEnum< RimIntersection::CrossSectionEnum >::setUp() {
  ...
  setDefault(RimIntersection::CS_POLYLINE);
}
...
RimIntersection::RimIntersection() {
  ...
  // -------------------------------------------------------------
  // CAF_PDM_InitField(&name,
  //                   "UserDescription",
  //                  QString("Intersection Name"), "Name", "", "", "");
  ...
  // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  // CAF_PDM_InitFieldNoDefault(&m_userPolyline,
  //                           "Points", "Points", "",
  //                           "Use Ctrl-C for copy and Ctrl-V for paste", "");

  // ---------------------------------------------------------------
  // CAF_PDM_InitField(&m_azimuthAngle,
  //                   "AzimuthAngle", 0.0, "Azimuth", "", "", "");

  // ---------------------------------------------------------------
  // m_azimuthAngle.uiCapability()->setUiEditorTypeName(
  //    caf::PdmUiDoubleSliderEditor::uiEditorTypeName());

  // ---------------------------------------------------------------
  // CAF_PDM_InitField(&m_dipAngle, "DipAngle", 90.0, "Dip", "", "", "");
  // m_dipAngle.uiCapability()->setUiEditorTypeName(
  //    caf::PdmUiDoubleSliderEditor::uiEditorTypeName());

  // ---------------------------------------------------------------
  // CAF_PDM_InitFieldNoDefault(&m_customExtrusionPoints,
  //                           "CustomExtrusionPoints", "", "", "", "");

  // CAF_PDM_InitFieldNoDefault(&m_twoAzimuthPoints,
  //                            "TwoAzimuthPoints", "Points", "",
  //                            "Use Ctrl-C for copy and Ctrl-V for paste", "");
  ...
  // caf::PdmUiPushButtonEditor::configureEditorForField(
  //    &inputPolyLineFromViewerEnabled);
...
// ===============================================================
//void RimIntersection::defineUiOrdering(QString uiConfigName,
//                                       caf::PdmUiOrdering& uiOrdering) {
//
//  // -------------------------------------------------------------
//  uiOrdering.add(&name);
//  caf::PdmUiGroup* geometryGroup =
//      uiOrdering.addNewGroup("Intersecting Geometry");
...
//
//    if (simulationWell()
//        && m_simulationWellBranchCenterlines.size() > 1) {
...
//RimIntersection::calculateValueOptions(
...
//    // -----------------------------------------------------------
//    if (coll) {
//      caf::PdmChildArrayField<RimSimWellInView*>& simWells =
//          coll->wells;
//
//      // ---------------------------------------------------------
...
//        options.push_back(caf::PdmOptionItemInfo(eclWell->name(),
//                                                 eclWell,
//                                                 false,
//                                                 simWellIcon));
//      }
...
//    // -----------------------------------------------------------
//    if (options.size() == 0) {
//      options.push_front(caf::PdmOptionItemInfo("None",
//                                                nullptr));
//    }
//
//    // -----------------------------------------------------------
...
//      options.push_back(caf::PdmOptionItemInfo(QString::number(bIdx + 1),
//                                               QVariant::fromValue(bIdx)));
//    }
//  }
//  return options;
//}
...
//RimIntersection::simulationWellCollection() const {
//
...
//
//    // -----------------------------------------------------------
//    double currentAzimuth = azimuthInRadians(
//        m_twoAzimuthPoints()[1] - m_twoAzimuthPoints()[0]);
//
...
//        m_twoAzimuthPoints()[1]
//            .getTransformedPoint(transFromOriginMat*rotMat*transToOriginMat);
//
...
RimIntersection::polyLines(cvf::Vec3d * flattenedPolylineStartPoint) const {

  // -------------------------------------------------------------
  if (flattenedPolylineStartPoint) {
    *flattenedPolylineStartPoint = cvf::Vec3d::ZERO;
  }
...
//      if (ownerCase) {
//        size_t dummy;
//        lines[0] = RigWellPath::clipPolylineStartAboveZ(
//            lines[0],
...
//
//      // ---------------------------------------------------------
//      if (0 <= branchIndexToUse
//          && branchIndexToUse
//              < static_cast<int>(m_simulationWellBranchCenterlines.size())) {
...
//
//    // -----------------------------------------------------------
//    if (flattenedPolylineStartPoint && lines.size()
//        && lines[0].size() > 1) {
//
//      (*flattenedPolylineStartPoint)[0] =
//          horizontalProjectedLengthAlongWellPathToClipPoint
//              - m_extentLength;
//
//      (*flattenedPolylineStartPoint)[2] =
//          lines[0][1].z(); // Depth of first point in first polyline
//    }
//
//  } else {

    if (flattenedPolylineStartPoint
        && lines.size() && lines[0].size() ) {
      (*flattenedPolylineStartPoint)[2] = lines[0][0].z();
      // Depth of first point in first polyline
    }
//  }
  return lines;
}
...
RimIntersection::polyLinesForExtrusionDirection() const {
...
// ===============================================================
//void RimIntersection::setPushButtonText(
//    bool buttonEnable,
...
// ===============================================================
//void RimIntersection::setBaseColor(
//    bool enable,
...
// ===============================================================
//void RimIntersection::defineEditorAttribute(
...
//  {
//    setBaseColor(inputPolyLineFromViewerEnabled,
//                 dynamic_cast<caf::PdmUiListEditorAttribute*>(attribute));
//  }
...
//  {
//    setBaseColor(inputTwoAzimuthPointsFromViewerEnabled,
//                 dynamic_cast<caf::PdmUiListEditorAttribute*>(attribute));
//  }
...
//  {
//    setBaseColor(inputExtrusionPointsFromViewerEnabled,
//                 dynamic_cast<caf::PdmUiListEditorAttribute*>(attribute));
//  }
//}
...
// ===============================================================
void
RimIntersection::appendPointToExtrusionDirection(const cvf::Vec3d& point) {
...
//  m_customExtrusionPoints.v().push_back(point);
  m_customExtrusionPoints.push_back(point);
...
//  {
//    cvf::Vec3d projectedPoint = cvf::Vec3d(point[0],
//                                           point[1],
//                                           m_twoAzimuthPoints.v()[0][2]);
//    m_twoAzimuthPoints.v().push_back(projectedPoint);
//
//    m_azimuthAngle = cvf::Math::toDegrees(azimuthInRadians(
//        m_twoAzimuthPoints()[1] - m_twoAzimuthPoints()[0]));
//
...
//
//    // MBHACK
//    iview->enableExtraDebugText(true);
//    iview->showExtraDebugText();
//    iview->showDefiningPoints();
//
//    // auto utm_vec = iview->transformToUtm();
//  }
}

// ===============================================================
//double RimIntersection::azimuthInRadians(cvf::Vec3d vec) {
//  return cvf::GeometryTools::getAngle(-cvf::Vec3d::Z_AXIS,
//                                      cvf::Vec3d::Y_AXIS, vec);
//}
