...
//
// This class stores vertices (nodes, coordinates), normals and
// a collection of PrimitiveSet to describe the geometry. Each
// PrimitiveSet describes an array of equal primitives (POINTS,
// LINES or TRIS).
//
// The geometry can be rendered in immediate mode (glBegin..),
// with vertex arrays or using VBO.
// ===============================================================
...
//    newGeo->m_primitiveSets
//        .push_back(const_cast<PrimitiveSet*>(m_primitiveSets.at(i)));
//  }
//
//  // -------------------------------------------------------------
...
//
// Will render from either client vertex arrays or buffer
// objects (VBOs).
//
// \note In order to get rendering from buffer objects, the buffer
// objects must already be created and uploaded through a previous
// call to createUploadBufferObjectsGPU().
//
// \warning Requires at least OpenGL 2.0
//void DrawableGeo::render(OpenGLContext* oglContext,
...
// ===============================================================
// Render the geometry using fixed function style of specifying
// the arrays.
//
// The main difference between this render path and that of
// render() is that this path will specify all client arrays
// or buffer objects using 'old style' glVertexArray(),
// glNormalArray() etc
//
// \warning Requires at least OpenGL 1.5 since it uses buffer
// objects.
//void DrawableGeo::renderFixedFunction(OpenGLContext* oglContext,
//                                      const MatrixState&) {
//
//  // -------------------------------------------------------------
...
//
//  // -------------------------------------------------------------
//  // Setup good old 'vertex arrays' for use with fixed function
//  // drawing
//  VertexBundleUsage bundleUsage;
...
//void DrawableGeo::
//createUploadBufferObjectsGPU(OpenGLContext* oglContext) {
//
//  // -------------------------------------------------------------
...
//
// \warning The OpenGL context in which the resources were
// created or a context that is being shared must be current
// in the calling thread.
//
// \warning In order to assure that the actual
// OpenGL resources get deleted, you must call
...
//void DrawableGeo::renderImmediateMode(OpenGLContext* oglContext,
//                                      const MatrixState&) {
//
//  // -------------------------------------------------------------
//#ifdef CVF_OPENGL_ES
...
// ===============================================================
// Get the number of triangles in the drawable. A quad is not 2
// triangles.
...
// ===============================================================
// Get the total number of OpenGL primitives
// (sum of lines, points, quads, etc.)
...
// ===============================================================
// Set the render mode (immediate, vertex array or VBO) to use
// when rendering the geometry
...
// ===============================================================
// Replace or add a vertex attribute to this DrawableGeo's vertex
// attribute set
//
// If a vertex attribute with the same name as the incoming
// attribute is already present, the existing attribute will
// be replaced.
...
// ===============================================================
// Add the given primitive set to the drawable. primitives
// cannot be NULL.
...
// \param indices Will receive the connectivity table for
// the specified face
void DrawableGeo::getFaceIndices(size_t indexOfFace,
                                 UIntArray* indices) const {
  ...
  for (ip = 0; ip < numPrimSets; ip++) {
    ...
    // -----------------------------------------------------------
    if (indexOfFace >= idxFirstPolyInPrimSet
        && indexOfFace < idxFirstPolyInPrimSet + primitiveFaceCount) {
...
// ===============================================================
// Sets the DrawableGeo object's geometry representation from a
// face list
//
// \param faceList Face list
//
// faceList contains number of items before each face
// connectivities. E.g. 3 0 1 2   3 2 3 1   3 2 1 3
//
// \note This method will use more temporary memory than
// strictly needed in order to optimize performance.
void DrawableGeo::setFromFaceList(const UIntArray& faceList) {
  ...
  triangleConnects->reserve(numFaceListEntries*3);
  // Usually too much, but temporary and will be squeezed if kept.
  ...
  // -------------------------------------------------------------
  // Check if the largest index used in the triangle connects
  // exceeds short representation
  if (triangleConnects->max() < std::numeric_limits<ushort>::max()) {
    ...
    for (j = 0; j < arraySize; j++) {
      shortIndices->
          set(j, static_cast<ushort>(triangleConnects->get(j)));
    }

    // -----------------------------------------------------------
    ref<PrimitiveSetIndexedUShort> prim =
        new PrimitiveSetIndexedUShort(PT_TRIANGLES);
    ...
    ref<PrimitiveSetIndexedUInt> prim =
        new PrimitiveSetIndexedUInt(PT_TRIANGLES);
...
// ===============================================================
// Setup a geometry with triangles from an
// array of vertices (3 vertices per triangle).
//
// This method sets the vertices to the passed vertexArray, and
// then creates one PrimitiveSet with one triangle for every three
// vertices in vertexArray.
void
DrawableGeo::setFromTriangleVertexArray(Vec3fArray* vertexArray) {
...
// ===============================================================
// Setup geometry with quads from an array of vertices
// (4 vertices per quad).
//
// This method sets the vertices to the passed vertexArray,
// and then creates one PrimitiveSet with two triangles for
// every four vertices in vertexArray.
//
...
//
// \warning Calling this function will delete all
// vertex related data except the vertex positions
void DrawableGeo::weldVertices(double weldDistance) {
  ...
  for (ip = 0; ip < numPrimSets; ip++) {
    ...
    // -----------------------------------------------------------
    ref<PrimitiveSetIndexedUInt> primSet =
        new PrimitiveSetIndexedUInt(primType);
...
// ===============================================================
// Merge a collection of drawable geometry objects into this
// drawable
//
// \param drawableGeos Collection of drawable geometries to
// be merged
//
// A new vertex array is created with the incoming vertex
// arrays appended to the existing contents.
//
// Primitives are copied and indices updated.
//
// \warning All other vertex attribute data such as normals,
// texture coordinates etc will be set to NULL
void DrawableGeo::mergeInto(
    const Collection<DrawableGeo>& drawableGeos) {
  ...
  for (i = 0; i < drawableGeos.size(); i++) {
    ...
    for (j = 0; j < otherDrawable->primitiveSetCount(); j++) {
      ...
      // ---------------------------------------------------------
      ref<PrimitiveSetIndexedUInt> prim =
          new PrimitiveSetIndexedUInt(primSet->primitiveType());
    ...
    // -----------------------------------------------------------
    // Append other drawable vertices vertex array and update
    // vertex index
    newVertexArr->copyData(otherVertices->ptr(),
                           otherVertices->size(),
                           currentVertexIndex);
...
// ===============================================================
// Merge a drawable geometry object with this drawable possibly
// with transformation.
//
// \param drawableGeo Drawable geometries to be merged
// \param transformation Transformation matrix used to modify
// vertices
//
// Vertices are converted if a transformation matrix is given.
// Vertex arrays are appended to the merged vertex array.
// Primitives are copied and indices updated.
//
// \warning All other vertex attribute data such as
// normals, texture coordinates etc will be set to NULL
void DrawableGeo::mergeInto(const DrawableGeo& drawableGeo,
  ...
  if (transformation) {
    ...
    for (j = 0; j < srcVertices->size(); j++) {
      ...
      // ---------------------------------------------------------
      // Transform to double vector to be able to do a transform
      // using a double matrix
    ...
    // Append other drawable vertices vertex array and update
    // vertex index
    newVertexArr->copyData(*srcVertices,
                           srcVertices->size(),
...
int DrawableGeo::convertFromUIntToUShort() {
  ...
  for (iPrim = 0; iPrim < numPrimitiveObjects; iPrim++) {
    ...
    if (vertexCount() < std::numeric_limits<ushort>::max() && primitiveSetUInt) {
      ...
      ref<PrimitiveSetIndexedUShortScoped> prim =
          new PrimitiveSetIndexedUShortScoped(primitive->primitiveType());

      prim->setIndices(indices.p(),
                       primitiveSetUIntScoped->scopeFirstElement(),
                       primitiveSetUIntScoped->scopeElementCount());
...
// ===============================================================
// Intersect the drawable geo with the ray and return the closest
// intersection point and the face hit
//
// Returns true if anything was hit.
bool DrawableGeo::rayIntersect(const Ray& ray,
...
// ===============================================================
bool DrawableGeo::rayIntersect(const Ray& ray,
                               Vec3dArray* intersectionPoints,
                               UIntArray* facesHit) const {
...
#pragma omp critical
        {
          ...
          CVF_TIGHT_ASSERT(i + accumulatedFaceCount
...
// ===============================================================
bool
DrawableGeo::rayIntersectCreateDetail(const Ray& ray,
                                      Vec3d* intersectionPoint,
                                      ref<HitDetail>* hitDetail) const {
