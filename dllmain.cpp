...
WELLINDEXCALCULATOR_API int computeWellIndices(const char* basepth,
  ...
  try {
    ...
    if ( !exists(gridpth.c_str()) ) {
      throw runtime_error("ComputeWellIndices: file .EGRID does not exist");
    }

    if ( *wellbore_radius <= 0 ){
      throw runtime_error("ComputeWellIndices: wellbore radius is negative");
    }

    // -------------------------------------------------------------
    printf("Initializing Grid object.\n");
    Reservoir::Grid::ECLGrid gridnew(gridpth);
    // NOTE: Make this work so we don't have to initialize a new grid
    // for every well!
    /*if (grid == NULL)
    {
    ...
    Reservoir::WellIndexCalculation::wicalc_rixx wicalc_rixx =
        Reservoir::WellIndexCalculation::wicalc_rixx(well_settings_, &gridnew);
