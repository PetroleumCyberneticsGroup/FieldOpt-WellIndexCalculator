...
#include "../../VizFwk/LibCore/cvfMath.h"
...
/// 
// -----------------------------------------------------------------
void StructGridInterface::characteristicCellSizes(double* iSize,
                                                  double* jSize,
                                                  double* kSize) const {
  ...
  if (m_characteristicCellSizeI == cvf::UNDEFINED_DOUBLE
    ...
    if (divisor > 0.0) {
      m_characteristicCellSizeI = Math::sqrt(iSize / divisor);
      m_characteristicCellSizeJ = Math::sqrt(jSize / divisor);
      m_characteristicCellSizeK = Math::sqrt(kSize / divisor);
    }
  }
