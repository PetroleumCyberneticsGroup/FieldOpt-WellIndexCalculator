...
RigWellLogExtractor::cellIntersectionInfosAlongWellPath() const {
...
RigWellLogExtractor::intersectedCellsGlobIdx() {
...
//--------------------------------------------------------------------
void
RigWellLogExtractor::insertIntersectionsInMap(
...
//--------------------------------------------------------------------
void
RigWellLogExtractor::populateReturnArrays(
    std::map<RigMDCellIdxEnterLeaveKey, HexIntersectionInfo > &uniqueIntersections) {

  // For same MD and same cell, remove enter/leave pairs, as
  // they only touch the wellpath, and should not contribute.
  {
    std::map<RigMDCellIdxEnterLeaveKey, HexIntersectionInfo >::iterator
        it1 = uniqueIntersections.begin();

    std::map<RigMDCellIdxEnterLeaveKey, HexIntersectionInfo >::iterator
        it2 = uniqueIntersections.begin();

    std::vector<std::map<RigMDCellIdxEnterLeaveKey, HexIntersectionInfo >::iterator>
  ...
  std::map<RigMDEnterLeaveCellIdxKey, HexIntersectionInfo > sortedUniqueIntersections;
  {
    ...
    while (it != uniqueIntersections.end()) {
      ...
      sortedUniqueIntersections.insert(
          std::make_pair(RigMDEnterLeaveCellIdxKey(it->first.measuredDepth,
  ...
  // Add points for the endpoint of the wellpath, if it starts/ends inside a cell
  {
    ...
    std::map<RigMDEnterLeaveCellIdxKey, HexIntersectionInfo >::iterator
    ...
    if (it != sortedUniqueIntersections.end() && !it->first.isEnteringCell) {
      ...
      sortedUniqueIntersections.
          insert(
          std::make_pair(RigMDEnterLeaveCellIdxKey(m_wellPath->m_measuredDepths[0],
                                                   true, firstLeavingPoint.m_hexIndex),
                         firstLeavingPoint));
    }

    // Add an intersection for the well endpoint possibly inside the last cell.
    std::map<RigMDEnterLeaveCellIdxKey, HexIntersectionInfo >::reverse_iterator
    ...
    while (it1 != sortedUniqueIntersections.end()) {
      ...
      if (RigMDEnterLeaveCellIdxKey::isProperCellEnterLeavePair(it1->first, it2->first)) {
        ...
        if (it21 != sortedUniqueIntersections.end()
          ...
          ++it11; ++it21;
          if (it21 != sortedUniqueIntersections.end()
              &&  RigMDEnterLeaveCellIdxKey::isProperCellEnterLeavePair(it11->first,
                                                                        it21->first)) {
