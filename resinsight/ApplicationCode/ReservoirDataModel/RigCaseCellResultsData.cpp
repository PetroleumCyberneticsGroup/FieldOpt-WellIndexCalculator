...
#include "../FileInterface/RifReaderEclipseOutput.h"
...
///
// -----------------------------------------------------------------
RigCaseCellResultsData::RigCaseCellResultsData(
    RigEclipseCaseData* ownerCaseData)
    : m_activeCellInfo(NULL) {
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::minMaxCellScalarValues(size_t scalarResultIndex,
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::posNegClosestToZero(size_t scalarResultIndex,
...
// RigCaseCellResultsData::cellScalarValuesHistogram(size_t scalarResultIndex) {
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::p10p90CellScalarValues(size_t scalarResultIndex,
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::meanCellScalarValues(size_t scalarResultIndex,
//                                                  double& meanValue) {
...
//                                                  size_t timeStepIndex,
//                                                  double& meanValue) {
//  m_statisticsDataCache[scalarResultIndex]->meanCellScalarValues(timeStepIndex,
//                                                                 meanValue);
//}
...
//RigCaseCellResultsData::uniqueCellScalarValues(size_t scalarResultIndex) {
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::sumCellScalarValues(size_t scalarResultIndex,
//                                                 double& sumValue) {
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::mobileVolumeWeightedMean(size_t scalarResultIndex,
//                                                      double& meanValue) {
...
// -----------------------------------------------------------------
size_t
RigCaseCellResultsData::timeStepCount(size_t scalarResultIndex) const {
...
RigCaseCellResultsData::cellScalarResults(size_t scalarResultIndex) const {
...
RigCaseCellResultsData::cellScalarResults( size_t scalarResultIndex ) {
...
RigCaseCellResultsData::cellScalarResults(size_t scalarResultIndex,
  ...
  std::cout << "scalarResultIndex: " << scalarResultIndex << std::endl;
...
///
// -----------------------------------------------------------------
size_t RigCaseCellResultsData::findScalarResultIndex(RiaDefines::ResultCatType type,
...
//      this->findScalarResultIndex(RiaDefines::STATIC_NATIVE, resultName);
//
//  if (scalarResultIndex == cvf::UNDEFINED_SIZE_T) {
//    scalarResultIndex =
//        this->findScalarResultIndex(RiaDefines::DYNAMIC_NATIVE, resultName);
//  }
//
//  if (scalarResultIndex == cvf::UNDEFINED_SIZE_T) {
//    scalarResultIndex =
//        this->findScalarResultIndex(RiaDefines::SOURSIMRL, resultName);
//  }
//
//  if (scalarResultIndex == cvf::UNDEFINED_SIZE_T) {
//    scalarResultIndex =
//        this->findScalarResultIndex(RiaDefines::GENERATED, resultName);
//  }
//
//  if (scalarResultIndex == cvf::UNDEFINED_SIZE_T) {
//    scalarResultIndex =
//        this->findScalarResultIndex(RiaDefines::INPUT_PROPERTY, resultName);
//  }
//
//  if(scalarResultIndex == cvf::UNDEFINED_SIZE_T) {
...
// -----------------------------------------------------------------
/// Adds an empty scalar set, and returns the scalarResultIndex to
/// it. If resultName already exists, it just returns the
/// scalarResultIndex to the existing result.
// -----------------------------------------------------------------
//size_t
//RigCaseCellResultsData::findOrCreateScalarResultIndex(RiaDefines::ResultCatType type,
...
// -----------------------------------------------------------------
//QStringList
//RigCaseCellResultsData::resultNames(RiaDefines::ResultCatType resType) const {
//  QStringList varList;
//  std::vector<RigEclipseResultInfo>::const_iterator it;
//  for (it = m_resultInfos.begin(); it != m_resultInfos.end(); ++it)
//  {
...
//  hasFlowFluxes = hasFlowFluxes &&
//      (dynResVarNames.contains("FLROILI+")
//          || dynResVarNames.contains("FLRGASI+"));
//
...
//RigCaseCellResultsData::allTimeStepDatesFromEclipseReader() const {
//
//  const RifReaderEclipseOutput* rifReaderOutput =
//      dynamic_cast<const RifReaderEclipseOutput*>(m_readerInterface.p());
//
//  if (rifReaderOutput) {
...
///
// -----------------------------------------------------------------
//QDateTime RigCaseCellResultsData::timeStepDate(size_t scalarResultIndex,
//                                               size_t timeStepIndex) const {
//
//  if (scalarResultIndex < m_resultInfos.size()
//      && m_resultInfos[scalarResultIndex].m_timeStepInfos.size() > timeStepIndex)
...
// -----------------------------------------------------------------
//std::vector<QDateTime>
//RigCaseCellResultsData::timeStepDates(size_t scalarResultIndex) const {
//
//  if (scalarResultIndex < m_resultInfos.size()) {
...
// -----------------------------------------------------------------
//std::vector<double>
//RigCaseCellResultsData::daysSinceSimulationStart(size_t scalarResultIndex) const
//{
//  if (scalarResultIndex < m_resultInfos.size()) {
...
///
// -----------------------------------------------------------------
//int RigCaseCellResultsData::reportStepNumber(size_t scalarResultIndex,
//                                             size_t timeStepIndex) const
//{
//  if (scalarResultIndex < m_resultInfos.size()
//      && m_resultInfos[scalarResultIndex].m_timeStepInfos.size() > timeStepIndex)
...
// -----------------------------------------------------------------
//std::vector<int>
//RigCaseCellResultsData::reportStepNumbers(size_t scalarResultIndex) const
//{
//  if (scalarResultIndex < m_resultInfos.size() )
...
//RigCaseCellResultsData::timeStepInfos(size_t scalarResultIndex) const {
//
//  if (scalarResultIndex < m_resultInfos.size())
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::setTimeStepInfos(size_t scalarResultIndex,
...
///
// -----------------------------------------------------------------
//size_t RigCaseCellResultsData::maxTimeStepCount(
//    size_t* scalarResultIndexWithMostTimeSteps) const {
//
//  size_t maxTsCount = 0;
...
///
// -----------------------------------------------------------------
//QString RigCaseCellResultsData::makeResultNameUnique(
//    const QString& resultNameProposal) const {
//
...
//    RiaDefines::ResultCatType type, const QString & resultName)
//{
//  size_t scalarResultIndex = this->findScalarResultIndex(type, resultName);
...
// -----------------------------------------------------------------
/// Removes all the actual numbers put into this object, and frees
/// up the memory. Does not touch the metadata in any way
// -----------------------------------------------------------------
void RigCaseCellResultsData::freeAllocatedResultsData() {
  ...
  for (size_t resultIdx = 0; resultIdx < m_cellScalarResults.size(); ++resultIdx) {
    for (size_t tsIdx = 0; tsIdx < m_cellScalarResults[resultIdx].size(); ++tsIdx) {
      // Using swap with an empty vector as that is the safest
      // way to really get rid of the allocated data in a vector
...
// -----------------------------------------------------------------
/// Make sure we have a result with given type and name, and make
// sure one "timestep" result vector for the static result values
// are allocated
// -----------------------------------------------------------------
//size_t
//RigCaseCellResultsData::addStaticScalarResult(RiaDefines::ResultCatType type,
//                                              const QString& resultName,
//                                              bool needsToBeStored,
...
// -----------------------------------------------------------------
//bool
//RigCaseCellResultsData::updateResultName(RiaDefines::ResultCatType resultType,
...
//RigCaseCellResultsData::getResultIndexableStaticResult(
...
//  size_t scalarResultIndexPorv = gridCellResults->findOrLoadScalarResult(
//      RiaDefines::STATIC_NATIVE, porvResultName);
//
//  if (scalarResultIndexPorv == cvf::UNDEFINED_SIZE_T) return nullptr;
//
//  const std::vector<double>* porvResults =
//      &(gridCellResults->cellScalarResults(scalarResultIndexPorv, 0));
//
...
//         globalCellIndex < reservoirCellCount; globalCellIndex++) {
//
...
// -----------------------------------------------------------------
//bool
//RigCaseCellResultsData::mustBeCalculated(size_t scalarResultIndex) const {
//
//  std::vector<RigEclipseResultInfo>::const_iterator it;
//  for (it = m_resultInfos.begin(); it != m_resultInfos.end(); ++it)
//  {
...
// -----------------------------------------------------------------
//void
//RigCaseCellResultsData::setMustBeCalculated(size_t scalarResultIndex) {
//
...
//  // Completion type
//  {
//    size_t completionTypeIndex = findScalarResultIndex(RiaDefines::DYNAMIC_NATIVE,
//                                                       RiaDefines::completionTypeResultName());
//    if (completionTypeIndex == cvf::UNDEFINED_SIZE_T) {
//      findOrCreateScalarResultIndex(RiaDefines::DYNAMIC_NATIVE,
//                                    RiaDefines::completionTypeResultName(), false);
//    }
//  }
//
//  // FLUX
//  {
//    size_t waterIndex = findScalarResultIndex(RiaDefines::DYNAMIC_NATIVE,
...
//    }
//
//    size_t oilIndex = findScalarResultIndex(RiaDefines::DYNAMIC_NATIVE,
...
//                                    false);
//    }
//    size_t gasIndex = findScalarResultIndex(RiaDefines::DYNAMIC_NATIVE,
...
//      addStaticScalarResult(RiaDefines::STATIC_NATIVE,
...
//  {
//    addStaticScalarResult(RiaDefines::STATIC_NATIVE,
//                          RiaDefines::combinedMultResultName(), false, 0);
//  }
//
...
// -----------------------------------------------------------------
//size_t
//RigCaseCellResultsData::findOrLoadScalarResult(const QString& resultName) {
//  size_t scalarResultIndex =
//      this->findOrLoadScalarResult(RiaDefines::STATIC_NATIVE, resultName);
//
//  if (scalarResultIndex == cvf::UNDEFINED_SIZE_T) {
//    scalarResultIndex =
//        this->findOrLoadScalarResult(RiaDefines::DYNAMIC_NATIVE, resultName);
//  }
//
//  if (scalarResultIndex == cvf::UNDEFINED_SIZE_T) {
//    scalarResultIndex =
//        this->findOrLoadScalarResult(RiaDefines::SOURSIMRL, resultName);
//  }
//
//  if (scalarResultIndex == cvf::UNDEFINED_SIZE_T) {
//    scalarResultIndex =
...
// -----------------------------------------------------------------
size_t
RigCaseCellResultsData::findOrLoadScalarResult(RiaDefines::ResultCatType type,
                                               const QString& resultName) {
...
//    {
//      // Trigger loading of SWAT, SGAS to establish time step count
//      // if no data has been loaded from file at this point
...
//    computeMobilePV();
  ...
  if (m_readerInterface.notNull())
  {
    ...
    size_t timeStepCount =
        this->infoForEachResultIndex()[scalarResultIndex].m_timeStepInfos.size();
...
//    if (eclReader)
//    {
//      size_t timeStepCount = this->infoForEachResultIndex()
//      [scalarResultIndex].m_timeStepInfos.size();
//
//      this->cellScalarResults(scalarResultIndex).resize(timeStepCount);
//
//      size_t i;
...
// -----------------------------------------------------------------
/// This method is intended to be used for multicase cross statistical
/// calculations, when we need process one timestep at a time, freeing
/// memory as we go.
///
// -----------------------------------------------------------------
//size_t RigCaseCellResultsData::findOrLoadScalarResultForTimeStep(
...
//    depthResultGridIndex = this->addStaticScalarResult(
//        RiaDefines::STATIC_NATIVE, "DEPTH", false, resultValueCount);
//    computeDepth = true;
//  }
//
//  if (dxResultGridIndex == cvf::UNDEFINED_SIZE_T) {
//    dxResultGridIndex = this->addStaticScalarResult(
//        RiaDefines::STATIC_NATIVE, "DX", false, resultValueCount);
//    computeDx = true;
//  }
//
//  if (dyResultGridIndex == cvf::UNDEFINED_SIZE_T) {
//    dyResultGridIndex = this->addStaticScalarResult(
//        RiaDefines::STATIC_NATIVE, "DY", false, resultValueCount);
//    computeDy = true;
//  }
//
//  if (dzResultGridIndex == cvf::UNDEFINED_SIZE_T) {
//    dzResultGridIndex = this->addStaticScalarResult(
//        RiaDefines::STATIC_NATIVE, "DZ", false, resultValueCount);
//    computeDz = true;
//  }
//
//  if (topsResultGridIndex == cvf::UNDEFINED_SIZE_T) {
//    topsResultGridIndex = this->addStaticScalarResult(
//        RiaDefines::STATIC_NATIVE, "TOPS", false, resultValueCount);
//    computeTops = true;
//  }
//
//  if (bottomResultGridIndex == cvf::UNDEFINED_SIZE_T) {
//    bottomResultGridIndex = this->addStaticScalarResult(
//        RiaDefines::STATIC_NATIVE, "BOTTOM", false, resultValueCount);
//    computeBottom = true;
//  }
//
...
//    if (computeDx) {
//      cvf::Vec3d cellWidth =
//          cell.faceCenter(cvf::StructGridInterface::NEG_I) -
...
//    if (computeDy) {
//      cvf::Vec3d cellWidth =
//          cell.faceCenter(cvf::StructGridInterface::NEG_J) -
...
//    if (computeDz) {
//      cvf::Vec3d cellWidth =
//          cell.faceCenter(cvf::StructGridInterface::NEG_K) -
...
//    if (computeTops) {
//      tops[0][cellIdx] =
//          cvf::Math::abs(cell.faceCenter(cvf::StructGridInterface::NEG_K).z());
//    }
//
//    if (computeBottom) {
//      bottom[0][cellIdx] =
//          cvf::Math::abs(cell.faceCenter(cvf::StructGridInterface::POS_K).z());
//    }
//  }
//}
...
void calculateConnectionGeometry(
    const RigCell& c1, const RigCell& c2, const std::vector<cvf::Vec3d>& nodes,
...
///
// -----------------------------------------------------------------
double halfCellTransmissibility(double perm, double ntg,
                                const cvf::Vec3d& centerToFace,
                                const cvf::Vec3d& faceAreaVec) {
...
///
// -----------------------------------------------------------------
double newtran(double cdarchy, double mult,
               double halfCellTrans, double neighborHalfCellTrans) {

  if (cvf::Math::abs(halfCellTrans) < 1e-15
      || cvf::Math::abs(neighborHalfCellTrans) < 1e-15) {
    return 0.0;
  }
...
///
// -----------------------------------------------------------------
typedef size_t(*ResultIndexFunction)(
    const RigActiveCellInfo* activeCellinfo, size_t reservoirCellIndex);

// -----------------------------------------------------------------
///
// -----------------------------------------------------------------

size_t directReservoirCellIndex(
    const RigActiveCellInfo* activeCellinfo, size_t reservoirCellIndex) {
...
size_t reservoirActiveCellIndex(
    const RigActiveCellInfo* activeCellinfo, size_t reservoirCellIndex) {
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::computeRiTransComponent(
...
//  size_t permResultIdx = findOrLoadScalarResult(RiaDefines::STATIC_NATIVE,
//                                                permCompName);
//  size_t ntgResultIdx = findOrLoadScalarResult(RiaDefines::STATIC_NATIVE,
//                                               "NTG");
//
//  bool hasNTGResults = ntgResultIdx != cvf::UNDEFINED_SIZE_T;
//
//  // Get the result index of the output
//  size_t riTransResultIdx = this->findScalarResultIndex(
//      RiaDefines::STATIC_NATIVE, riTransComponentResultName);
...
//
//  for (size_t nativeResvCellIndex = 0;
//       nativeResvCellIndex < m_ownerMainGrid->globalCellArray().size();
//       nativeResvCellIndex++) {
//
...
//      {
//        cvf::Vec3d centerToFace = neighborCell.faceCenter(
//            cvf::StructGridInterface::oppositeFace(faceId)) - neighborCell.center();
//
...
//      this->findScalarResultIndex(RiaDefines::STATIC_NATIVE,
//                                  RiaDefines::combinedRiTranResultName());
//
//  if (m_ownerMainGrid->nncData()->
//      staticConnectionScalarResult(riCombTransScalarResultIndex))
//    return;
//
//  double cdarchy = darchysValue();
//
...
//      m_ownerMainGrid->nncData()->makeStaticConnectionScalarResult(
//          RigNNCData::propertyNameRiCombTrans());
//
//  m_ownerMainGrid->nncData()->setScalarResultIndex(
//      RigNNCData::propertyNameRiCombTrans(), riCombTransScalarResultIndex);
//
//  std::vector<double> * ntgResults     = NULL;
//  if (hasNTGResults) {
//    ntgResults = &(this->cellScalarResults(ntgResultIdx)[0]);
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::computeRiMULTComponent(
//    const QString& riMultCompName) {
//
//  // Set up which component to compute
...
//
//  // Get the result count, to handle that one of them might be globally defined
//  CVF_ASSERT(this->cellScalarResults(riTransResultIdx)[0].size() ==
//      this->cellScalarResults(transResultIdx)[0].size());
//
...
//                                  RiaDefines::combinedTransmissibilityResultName());
//
//  if (m_ownerMainGrid->nncData()->staticConnectionScalarResult(riCombMultScalarResultIndex))
//    return;
//
...
///
// -----------------------------------------------------------------
//void RigCaseCellResultsData::computeRiTRANSbyAreaComponent(
//    const QString& riTransByAreaCompResultName) {
//  // Set up which component to compute
//
//  cvf::StructGridInterface::FaceType faceId = cvf::StructGridInterface::NO_FACE;
//  QString transCompName;
...
//      findOrLoadScalarResult(RiaDefines::STATIC_NATIVE,
//                             transCompName);
//
//  // Get the result index of the output
//  size_t riTranByAreaScResIdx =
//      this->findScalarResultIndex(RiaDefines::STATIC_NATIVE,
//                                  riTransByAreaCompResultName);
...
//
//  // Set up result index function pointers
//  ResultIndexFunction resValIdxFunc =
//      isUsingResIdx ? &reservoirActiveCellIndex : &directReservoirCellIndex;
//
//  const RigActiveCellInfo* activeCellInfo = this->activeCellInfo();
//  const std::vector<cvf::Vec3d>& nodes = m_ownerMainGrid->nodes();
//
...
//          grid->mainGrid()->findFaultFromCellIndexAndCellFace(nativeResvCellIndex, faceId);
//      bool isOnFault = fault;
//
//      cvf::Vec3d faceAreaVec;
//
...
//                                  RiaDefines::combinedTransmissibilityResultName());
//
//  if (m_ownerMainGrid->nncData()->staticConnectionScalarResult(riCombTransByAreaScResIdx))
//    return;
//
...
//      m_ownerMainGrid->nncData()->connections();
//
...
//      this->findScalarResultIndex(RiaDefines::DYNAMIC_NATIVE,
//                                  RiaDefines::completionTypeResultName());
//
...
//  }
//
//  std::vector<double>& completionTypeResult =
//      this->cellScalarResults(completionTypeResultIndex, timeStep);
//
...
//
//  RimCompletionCellIntersectionCalc::calculateCompletionTypeResult(
//      project, eclipseCase, m_ownerMainGrid, completionTypeResult,
//      timeStepDate);
//}
...
//
//  porvResults   = RigCaseCellResultsData::getResultIndexableStaticResult(
//      this->activeCellInfo(), this, "PORV", porvDataTemp);
//
//  swcrResults   = RigCaseCellResultsData::getResultIndexableStaticResult(
//      this->activeCellInfo(), this, "SWCR", swcrDataTemp);
//
//  multpvResults = RigCaseCellResultsData::getResultIndexableStaticResult(
//      this->activeCellInfo(), this, "MULTPV", multpvDataTemp);
//
//  CVF_ASSERT(!porvResults->empty());
//
//  size_t mobPVIdx =
//      this->findOrCreateScalarResultIndex(RiaDefines::STATIC_NATIVE,
...
//    for (size_t vIdx = 0; vIdx < porvResults->size(); ++vIdx) {
//      mobPVResults[vIdx] =
//          (*multpvResults)[vIdx] * (*porvResults)[vIdx] * (1.0 - (*swcrResults)[vIdx]);
//    }
//  }
//}
