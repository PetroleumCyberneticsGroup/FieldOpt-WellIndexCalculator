    ...
    double *x, double *y, double* fractionAlongLine1, double* fractionAlongLine2)
{
  ...
  if (fabs(numera) < EPS && fabs(numerb) < EPS && fabs(denom) < EPS)
  {
    ...
    *x = 0;
    ...
    if ((0.0 - l1NormalizedTolerance) <=
        normDist13 && normDist13 <= (1.0 +l1NormalizedTolerance) ) pt3IsInside = true;

    if ((0.0 - l1NormalizedTolerance) <=
        normDist14 && normDist14 <= (1.0 +l1NormalizedTolerance) ) pt4IsInside = true;
  ...
  *fractionAlongLine2 = mub;
  ...
  if (mua < 0 - l1NormalizedTolerance || 1 + l1NormalizedTolerance < mua
      || mub < 0 - l2NormalizedTolerance ||  1 + l2NormalizedTolerance < mub)
  {
...
GeometryTools::inPlaneLineIntersect3D(const cvf::Vec3d& planeNormal,
  ...
  IntersectionStatus intersectionStatus =
      inPlaneLineIntersect(p1[X], p1[Y], p2[X], p2[Y],
                           p3[X], p3[Y], p4[X], p4[Y],
                           l1NormTol, l2NormTol, &x, &y,
                           fractionAlongLine1, fractionAlongLine2);
...
/// If projection is out side the line segment, the end of line is
/// returned
// -----------------------------------------------------------------
...
// -----------------------------------------------------------------
double GeometryTools::linePointSquareDist(const cvf::Vec3d& p1,
                                          const cvf::Vec3d& p2,
                                          const cvf::Vec3d& p3)
{
  cvf::Vec3d v31 = p3 - p1;
...
int GeometryTools::intersectLineSegmentTriangle(
    const cvf::Vec3d p0, const cvf::Vec3d p1,
...
cvf::Vec3d barycentricCoordsExperiment(const cvf::Vec3d& t0,
 const cvf::Vec3d& t1, const cvf::Vec3d& t2, const cvf::Vec3d& p)
{
...
// -----------------------------------------------------------------
inline double TriArea2D(double x1, double y1,
                        double x2, double y2,
                        double x3, double y3)
{
...
// -----------------------------------------------------------------
cvf::Vec3d
GeometryTools::barycentricCoords(const cvf::Vec3d& t0, const cvf::Vec3d& t1,
                                 const cvf::Vec3d& t2, const cvf::Vec3d& p) {
...
void GeometryTools::addMidEdgeNodes(
    ...
    const cvf::Vec3dArray& nodes, EdgeSplitStorage& edgeSplitStorage,
  ...
  for (it = polygon->begin(); it != polygon->end(); ++it)
  {
    ...
    if (!edgeSplitStorage.findSplitPoint(it->first,
                                         it2->first,
                                         &midPointIndex)) {
      ...
      edgeSplitStorage.addSplitPoint(it->first,
                                     it2->first,
                                     midPointIndex);
    }
...
cvf::Vec3d GeometryTools::polygonAreaNormal3D(const std::vector<cvf::Vec3d>& polygon)
{
  ...
  switch (pSize)
  {
    ...
    {
      /// JJS:
      // This is possibly not the fastest approach with large polygons,
      // where a scaled projections approach would be better, but I suspect
      // this (simpler) approach is faster for small polygons, as long as
      // we do not have the polygon normal up front.
      //
...
bool EarClipTesselator::calculateTriangles( std::vector<size_t>* triangleIndices ) {
  ...
  // We want m_polygonIndices to be a counter-clockwise
  // polygon to make the validation test work
  ...
  // If we loop two times around polygon without
  // clipping a single triangle we are toast.
...
bool EarClipTesselator::isTriangleValid(
  ...
  for (c = m_polygonIndices.begin(); c != m_polygonIndices.end(); ++c) {
    ...
    // The polygon points that actually make up the triangle candidate
    // does not count (but the same points on different positions in
    // the polygon does!
    // Except those one off the triangle, that references the start or
    // end of the triangle)

    if ( (c == u) || (c == v) || (c == w)) continue;

    // Originally the below tests was not included
    // which resulted in missing triangles sometimes

    outside = w;
    ++outside;
    if (outside == m_polygonIndices.end()) {
      outside = m_polygonIndices.begin();
    }
    ...
    outside = u;
    if (outside == m_polygonIndices.begin()) {
      outside = m_polygonIndices.end();
    }
    --outside;
...
// -----------------------------------------------------------------
/// This needs to be rewritten because we need to test for crossing
/// edges, not only point inside. In addition the test for polygon
// -----------------------------------------------------------------
