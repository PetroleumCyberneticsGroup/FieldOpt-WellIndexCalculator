...
double RigWellPath::wellPathAzimuthAngle(const cvf::Vec3d& position) const {
  ...
  for (size_t i = 1; i < m_wellPathPoints.size(); i++)
  {
    ...
    double candidateDistance =
        cvf::GeometryTools::linePointSquareDist(p1, p2, position);
...
/// 
// -----------------------------------------------------------------
void RigWellPath::twoClosestPoints(const cvf::Vec3d& position,
                                   cvf::Vec3d* p1, cvf::Vec3d* p2) const {
...
RigWellPath::clippedPointSubset(double startMD, double endMD) const {
...
RigWellPath::wellPathPointsIncludingInterpolatedIntersectionPoint(
    double intersectionMeasuredDepth) const {

  std::vector<cvf::Vec3d> points;
  if (m_measuredDepths.empty()) return points;

  cvf::Vec3d interpolatedWellPathPoint =
      interpolatedPointAlongWellPath(intersectionMeasuredDepth);
