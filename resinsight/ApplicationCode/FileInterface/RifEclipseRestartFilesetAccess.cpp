...
void RifEclipseRestartFilesetAccess::setRestartFiles(const QStringList& fileSet) {
  ...
  // To make sure they are sorted in increasing *.X000N order.
  // Hack. Should probably be actual time stored on file.
  m_fileNames.sort();
...
/// 
//--------------------------------------------------------------------
void RifEclipseRestartFilesetAccess::setTimeSteps(
...
/// Get the time steps
//--------------------------------------------------------------------
void RifEclipseRestartFilesetAccess::timeSteps(
...
//--------------------------------------------------------------------
void RifEclipseRestartFilesetAccess::resultNames(QStringList* resultNames,
                                                 std::vector<size_t>* resultDataItemCounts)
{
  ...
  RifEclipseOutputFileTools::findKeywordsAndItemCount(m_ecl_files,
                                                      resultNames,
                                             ...
                                             std::vector<double>* values)
{
  ...
  size_t fileGridCount = ecl_file_get_num_named_kw(m_ecl_files[timeStep],
  ...
  for (i = 0; i < fileGridCount; i++) {
    ...
    if (!RifEclipseOutputFileTools::keywordData(m_ecl_files[timeStep],
                                                resultName, i,
                                                &gridValues)) {
      return false;
    }
