...
bool RifEclipseUnifiedRestartFileAccess::openFile() {
  ...
  if (!m_ecl_file) {
    QString indexFileName =
        RifEclipseOutputFileTools::createIndexFileName(m_filename);
...
//          m_ecl_file = ecl_file_fast_open(m_filename.toLatin1().data(),
//                                          indexFileName.toLatin1().data(),
//                                          ECL_FILE_CLOSE_STREAM);
          if (!m_ecl_file) {
            RiaLogging::error(QString("Failed to open file %1 using index file.")
                                  .arg(m_filename));
            RiaLogging::info(QString("Will try to open file without index file."));
          }
          else {
            RiaLogging::info(QString("Imported file %1 using index file.")
                                 .arg(m_filename));
          }
        }
      }
    ...
    if (!m_ecl_file)
    {
      m_ecl_file = ecl_file_open(m_filename.toLatin1().data(),
                                 ECL_FILE_CLOSE_STREAM);
      if (!m_ecl_file)
      {
...
// -----------------------------------------------------------------
void RifEclipseUnifiedRestartFileAccess::resultNames(QStringList* resultNames,
                                                     std::vector<size_t>* resultDataItemCounts)
{
  if (openFile())
  {
    ...
    RifEclipseOutputFileTools::findKeywordsAndItemCount(
        ...
        resultNames, resultDataItemCounts);
  }
}
...
bool RifEclipseUnifiedRestartFileAccess::results(const QString& resultName,
                                                 size_t timeStep, size_t gridCount,
  ...
  for (size_t i = 0; i < gridCount; i++)
  {
    ...
    int namedKeywordCount = ecl_file_get_num_named_kw(m_ecl_file,
                                                      resultName.toLatin1().data());
...
bool RifEclipseUnifiedRestartFileAccess::dynamicNNCResults(const ecl_grid_type* grid,
  ...
  ecl_file_view_type* summaryView =
      ecl_file_get_restart_view(m_ecl_file,
                                static_cast<int>(timeStep),
                                0, 0, 0);

  RifEclipseOutputFileTools::transferNncFluxData(grid,
                                                 summaryView,
                                                 waterFlux,
                                                 oilFlux,
                                                 gasFlux);
...
RifEclipseUnifiedRestartFileAccess::availablePhases() const {
