...
/// 
// -----------------------------------------------------------------
void RifEclipseOutputFileTools::findKeywordsAndItemCount(
    std::vector<ecl_file_type*> ecl_files,
  ...
  for (auto reportStep : reportSteps) {
    for (auto keywordItemCount : reportStep.m_keywords.keywordsWithAggregatedItemCount()) {
      ...
      auto it = std::find(foundKeywords.begin(),
                          foundKeywords.end(),
                          keywordItemCount.first);

      if (it == foundKeywords.end()) {
        foundKeywords.push_back(KeywordItemCounter(keywordItemCount.first,
                                                   keywordItemCount.second));
      }
      else {
...
void getDayMonthYear(const ecl_kw_type* intehead_kw,
...
// -----------------------------------------------------------------
void RifEclipseOutputFileTools::timeSteps(ecl_file_type* ecl_file,
...
/// 
// -----------------------------------------------------------------
bool RifEclipseOutputFileTools::keywordData(ecl_file_type* ecl_file,
...
// -----------------------------------------------------------------
QString RifEclipseOutputFileTools::firstFileNameOfType(const QStringList& fileSet,
...
// -----------------------------------------------------------------
QStringList RifEclipseOutputFileTools::filterFileNamesOfType(
...
// -----------------------------------------------------------------
bool RifEclipseOutputFileTools::findSiblingFilesWithSameBaseName(
...
/// 
// -----------------------------------------------------------------
void RifEclipseOutputFileTools::readGridDimensions(
    const QString& gridFileName,
    std::vector< std::vector<int> >& gridDimensions) {

  // bootstrap ecl_grid instance
  ecl_grid_type * grid         = ecl_grid_alloc(gridFileName.toLatin1().data());
  // get a list of all the lgr names.
  stringlist_type * lgr_names  = ecl_grid_alloc_lgr_name_list( grid );
...
//         grid_filename, stringlist_get_size( lgr_names ));
  ...
  for (int lgr_nr = 0; lgr_nr < stringlist_get_size( lgr_names); lgr_nr++) {
    // get the ecl_grid instance of the lgr - by name.
    ecl_grid_type * lgr_grid  = ecl_grid_get_lgr(grid,
                                                 stringlist_iget(lgr_names, lgr_nr ));

    int nx,ny,nz,active_size;
    // get some size info from this lgr.
    ecl_grid_get_dims(lgr_grid, &nx, &ny, &nz, &active_size);
...
// -----------------------------------------------------------------
/// Returns the following integer values from the first INTEHEAD
/// keyword found
///  1  : METRIC
///  2  : FIELD
///  3  : LAB
...
/// 
// -----------------------------------------------------------------
cvf::ref<RifEclipseRestartDataAccess>
RifEclipseOutputFileTools::createDynamicResultAccess(const QString& fileName)
{
  ...
  QString unrstFileName = RifEclipseOutputFileTools::firstFileNameOfType(
  ...
  if (unrstFileName.size() > 0) {
    ...
    QStringList restartFiles =
        RifEclipseOutputFileTools::filterFileNamesOfType(filesWithSameBaseName,
                                                         ECL_RESTART_FILE);
...
// -----------------------------------------------------------------
QString
RifEclipseOutputFileTools::createIndexFileName(const QString& resultFileName) {
...
RifEclipseOutputFileTools::findAvailablePhases(ecl_file_type* ecl_file) {
...
void RifEclipseOutputFileTools::transferNncFluxData(const ecl_grid_type* grid,
  ...
  if (nnc_geo) {
    ecl_nnc_data_type* waterFluxData = ecl_nnc_data_alloc_wat_flux(grid,
                                                                   nnc_geo,
                                                                   summaryView);
    if (waterFluxData) {
    ...
    ecl_nnc_data_type* oilFluxData = ecl_nnc_data_alloc_oil_flux(grid,
                                                                 nnc_geo,
                                                                 summaryView);
    if (oilFluxData) {
      const double* oilFluxValues = ecl_nnc_data_get_values(oilFluxData);
      oilFlux->insert(oilFlux->end(),
                      &oilFluxValues[0],
                      &oilFluxValues[ecl_nnc_data_get_size(oilFluxData)]);
      ecl_nnc_data_free(oilFluxData);
    }

    ecl_nnc_data_type* gasFluxData = ecl_nnc_data_alloc_gas_flux(grid,
                                                                 nnc_geo,
                                                                 summaryView);
    if (gasFluxData) {
      const double* gasFluxValues = ecl_nnc_data_get_values(gasFluxData);
      gasFlux->insert(gasFlux->end(),
                      &gasFluxValues[0],
                      &gasFluxValues[ecl_nnc_data_get_size(gasFluxData)]);
...
void RifEclipseOutputFileTools::createReportStepsMetaData(
    std::vector<ecl_file_type*> ecl_files, std::vector<RifRestartReportStep>* reportSteps)
{
    ...
    for (int reportStepIndex = 0; reportStepIndex < reportStepCount; reportStepIndex++) {
      ...
      if (restart_header)
      {
...

        {
          ...
          // Set Date
          {
            QDateTime reportDateTime(QDate(restart_header->year,
                                           restart_header->month,
                                           restart_header->day));
          ...
          for (int iKey = 0; iKey < numKeywords; iKey++)
          {
            ...
            {
              ecl_data_type dataType = ecl_file_iget_named_data_type(ecl_file,
                                                                     kw, iOcc);
              ecl_type_enum dataTypeEmum = ecl_type_get_type(dataType);

              if (dataTypeEmum != ECL_DOUBLE_TYPE && dataTypeEmum
                  != ECL_FLOAT_TYPE && dataTypeEmum != ECL_INT_TYPE) {
                continue;
              }
