...
//RifEclipseInputFileTools::readProperties(const QString& fileName,
...
//        ecl_kw_fscanf_alloc_current_grdecl__(gridFilePointer,
//                                             false,
...
//--------------------------------------------------------------------
bool
RifEclipseInputFileTools::readProperty(const QString& fileName,
                                       RigEclipseCaseData* caseData,
  ...
  bool isOk = false;
//  if (eclipseKeywordData) {
//    isOk = readDataFromKeyword(eclipseKeywordData,
//                               caseData,
//                               resultName);
//    ecl_kw_free(eclipseKeywordData);
//  }
...
//      caseData->results(RiaDefines::MATRIX_MODEL)->cellScalarResults(resultIndex);
//
...
//
//--------------------------------------------------------------------
void RifEclipseInputFileTools::findKeywordsOnFile(const QString &fileName,
...
RifEclipseInputFileTools::invalidPropertyDataKeywords() {
...
/// 
//--------------------------------------------------------------------
//bool RifEclipseInputFileTools::writePropertyToTextFile(const QString& fileName,
...
//
//  size_t resultIndex =
//      eclipseCase->results(RiaDefines::MATRIX_MODEL)->findScalarResultIndex(resultName);
...
//      eclipseCase->results(RiaDefines::MATRIX_MODEL)->cellScalarResults(resultIndex);
...
//                                                           eclipseCase->mainGrid()->gridIndex(),
//                                                           timeStep,
//                                                           resultDefinition);
...
/// 
//--------------------------------------------------------------------
void RifEclipseInputFileTools::writeDataToTextFile(QFile* file,
                                                   const QString& eclipseKeyWord,
...
//  caf::ProgressInfo pi(resultData.size(),
// QString("Writing data to file %1").arg(file->fileName()) );
...
/// 
//--------------------------------------------------------------------
void RifEclipseInputFileTools::findGridKeywordPositions(
    const std::vector< RifKeywordAndFilePos >& keywordsAndFilePos,
    qint64* coordPos, qint64* zcornPos, qint64* specgridPos,
    qint64* actnumPos, qint64* mapaxesPos)
{
                                          ...
                                          cvf::Collection<RigFault>* faults)
{
  ...
  for (size_t i = 0; i < fileKeywords.size(); i++) {
    if (fileKeywords[i].keyword.compare(editKeyword,
                                        Qt::CaseInsensitive) == 0) {
      return;
    }
    else if (fileKeywords[i].keyword.compare(faultsKeyword,
                                             Qt::CaseInsensitive) != 0) {
      continue;
    }
...
/// 
//--------------------------------------------------------------------
void RifEclipseInputFileTools::parseAndReadFaults(const QString& fileName,
                                                  cvf::Collection<RigFault>* faults)
{
  QFile data(fileName);
  if (!data.open(QFile::ReadOnly))
...
//{
//  size_t resultIndex =
//      reservoir->results(RiaDefines::MATRIX_MODEL)->findScalarResultIndex(newResultName);
//  if (resultIndex == cvf::UNDEFINED_SIZE_T)
//  {
//    resultIndex =
//        reservoir->results(RiaDefines::MATRIX_MODEL)->findOrCreateScalarResultIndex(RiaDefines::INPUT_PROPERTY, newResultName, false);
//  }
//
//  return resultIndex;
//}
