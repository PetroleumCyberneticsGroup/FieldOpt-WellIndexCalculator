...
bool RifReaderEclipseOutput::transferGeometry(const ecl_grid_type* mainEclGrid,
  ...
  RigActiveCellInfo* activeCellInfo =
      eclipseCase->activeCellInfo(RiaDefines::MATRIX_MODEL);

  RigActiveCellInfo* fractureActiveCellInfo =
      eclipseCase->activeCellInfo(RiaDefines::FRACTURE_MODEL);
...
//------------------------------------------------------------------
bool RifReaderEclipseOutput::open(const QString& fileName,
                                  RigEclipseCaseData* eclipseCase)
{
  CVF_ASSERT(eclipseCase);
...
//
//  RigCaseCellResultsData* matrixModelResults =
//      m_eclipseCase->results(RiaDefines::MATRIX_MODEL);
...
//                            .arg(allTimeSteps().size()).arg(sourSimTimeSteps.size()));
//
//      return;
//    }
//
...
//        if (!isEclipseAndSoursimTimeStepsEqual(timeStepInfos[i].m_date,
//                                               sourSimTimeSteps[indexOnFile])) {
//          isTimeStampsEqual = false;
//        }
//      } else {
//        RiaLogging::error(QString("HDF: Time step count mismatch, Eclipse : %1 ; HDF : %2 ")
//                              .arg(timeStepInfos.size()).arg(sourSimTimeSteps.size()));
//
...
//    }
//
//    timeStepInfos = RigEclipseTimeStepInfo::createTimeStepInfos(
//        sourSimTimeSteps, reportNumbers, daysSinceSimulationStart);
//  }
//
//  QStringList resultNames = hdf5ReaderInterface->propertyNames();
//  for (int i = 0; i < resultNames.size(); ++i) {
//
//    size_t resIndex = matrixModelResults->findOrCreateScalarResultIndex(
//        RiaDefines::SOURSIMRL, resultNames[i], false);
...
///
//------------------------------------------------------------------
void RifReaderEclipseOutput::setFileDataAccess(
    RifEclipseRestartDataAccess* restartDataAccess) {
...
///
//------------------------------------------------------------------
void RifReaderEclipseOutput::importFaults(const QStringList& fileSet,
                                          cvf::Collection<RigFault>* faults) {
...
///
// -----------------------------------------------------------------
void RifReaderEclipseOutput::transferStaticNNCData(
    ...
    const ecl_grid_type* mainEclGrid, RigMainGrid* mainGrid)
{
  ...
  std::vector< std::vector<double> >& waterFluxData =
      mainGrid->nncData()->makeDynamicConnectionScalarResult(
          RigNNCData::propertyNameFluxWat(), timeStepCount);

  std::vector< std::vector<double> >& oilFluxData =
      mainGrid->nncData()->makeDynamicConnectionScalarResult(
          RigNNCData::propertyNameFluxOil(), timeStepCount);

  std::vector< std::vector<double> >& gasFluxData =
      mainGrid->nncData()->makeDynamicConnectionScalarResult(
          RigNNCData::propertyNameFluxGas(), timeStepCount);
...
bool RifReaderEclipseOutput::readActiveCellInfo()
{
  ...
  QString egridFileName =
      RifEclipseOutputFileTools::firstFileNameOfType(m_filesWithSameBaseName,
                                                     ECL_EGRID_FILE);
      ...
      for (size_t gridIdx = 0; gridIdx < static_cast<size_t>(actnumKeywordCount); gridIdx++) {
        ...
        for (size_t i = 0; i < actnumValues.size(); i++) {
          if (actnumValues[i] == 1 || actnumValues[i] == 3) {
            activeCellInfo->setCellResultIndex(cellIdx,
                                               globalActiveMatrixIndex++);
            activeMatrixIndex++;
          }

          if (actnumValues[i] == 2 || actnumValues[i] == 3) {
            fractureActiveCellInfo->setCellResultIndex(cellIdx,
                                                       globalActiveFractureIndex++);
            activeFractureIndex++;
          }
          cellIdx++;
        }
...
//      m_eclipseCase->results(RiaDefines::FRACTURE_MODEL);
//
...
//        size_t resIndex =
//            matrixModelResults->findOrCreateScalarResultIndex(RiaDefines::DYNAMIC_NATIVE,
//                                                              matrixResultNames[i], false);
//        matrixModelResults->setTimeStepInfos(resIndex, timeStepInfos);
//      }
//    }
//
...
//          validKeywordsForPorosityModel(resultNames, resultNamesDataItemCounts,
//                                        m_eclipseCase->activeCellInfo(RiaDefines::MATRIX_MODEL),
//                                        m_eclipseCase->activeCellInfo(RiaDefines::FRACTURE_MODEL),
...
//        size_t resIndex =
//            fractureModelResults->findOrCreateScalarResultIndex(RiaDefines::DYNAMIC_NATIVE,
//                                                                fractureResultNames[i],
//                                                                false);
...
//        filesUsedToFindAvailableKeywords,
//        &resultNames,
//        &resultNamesDataItemCounts);
//
...
//      for (int i = 0; i < matrixResultNames.size(); ++i) {
//        size_t resIndex =
//            matrixModelResults->findOrCreateScalarResultIndex(RiaDefines::STATIC_NATIVE,
//                                                              matrixResultNames[i],
//                                                              false);
...
//      for (int i = 0; i < fractureResultNames.size(); ++i) {
//        size_t resIndex =
//            fractureModelResults->findOrCreateScalarResultIndex(RiaDefines::STATIC_NATIVE,
//                                                                fractureResultNames[i],
//                                                                false);
...
void RifReaderEclipseOutput::ensureDynamicResultAccessIsPresent() {
  if (m_dynamicResultsAccess.isNull()) {
    m_dynamicResultsAccess =
        RifEclipseOutputFileTools::createDynamicResultAccess(m_fileName);
  }
}
...
//------------------------------------------------------------------
bool
RifReaderEclipseOutput::staticResult(const QString& result,
  ...
  if(m_ecl_init_file) {
    ...
    for (i = 0; i < numOccurrences; i++)
    {
      ...
      RifEclipseOutputFileTools::keywordData(m_ecl_init_file,
                                             result, i,
                                             &partValues);
...
///
//------------------------------------------------------------------
//void RifReaderEclipseOutput::sourSimRlResult(const QString& result,
//                                             size_t stepIndex,
//                                             std::vector<double>* values) {
//  values->clear();
//
...
//
//  {
//    RigActiveCellInfo* fracActCellInfo =
//        m_eclipseCase->activeCellInfo(RiaDefines::MATRIX_MODEL);
...
//------------------------------------------------------------------
/// Helper struct to store info on how a well-to-grid connection
/// contributes to the position of well segments without any
/// connections.
//------------------------------------------------------------------
                                           ...
                                           const char* wellNameForErrorMsgs )
{
  ...
  // If a well is defined in fracture region, the K-value
  // is from (cellCountK - 1) -> cellCountK*2 - 1
  ...
  // See description for keyword ICON at page 54/55
  // of Rile Formats Reference Manual 2010.2
  /*
  ...
  */
  ...
  // The K value might also be -1. It is not yet known why, or what
  // it is supposed to mean, but for now we will interpret as 0.
  // TODO: Ask Joakim Haave regarding this.
  if (cellK < 0) {
//    cvf::Trace::show("Well Connection for grid "
//                         + cvf::String(grid->gridName())
//                         + "\n - Detected negative K value (K="
//                         + cvf::String(cellK) + ") for well : "
//                         + cvf::String(wellName) + " K clamped to 0");
  ...
  if (cellK >= static_cast<int>(grid->cellCountK()))
  {
    ...
    if (wellNameForErrorMsgs) {
      cvf::Trace::show("Well Connection for grid "
                           + cvf::String(grid->gridName())
                           + "\n - Ignored connection with invalid K value (K="
                           + cvf::String(cellK) + ", max K = "
                           + cvf::String(maxCellK) + ") for well : "
                           + cvf::String(wellNameForErrorMsgs));
    }
...
//------------------------------------------------------------------
RigWellResultPoint
RifReaderEclipseOutput::createWellResultPoint(const RigGridBase* grid,
                                              const well_conn_type* ert_connection,
  ...
  size_t gridCellIndex = localGridCellIndexFromErtConnection(grid,
                                                             ert_connection,
                                                             wellName);
...
//        RiaEclipseUnitTools::convertSurfaceGasFlowRateToOilEquivalents(
//            m_eclipseCase->unitsType(), gasRate);
  }
...
//------------------------------------------------------------------
/// Inverse distance interpolation of the supplied points and
/// distance weights for the contributing points which are closest
/// above, and closest below
//------------------------------------------------------------------
cvf::Vec3d
interpolate3DPosition(
    const std::vector<SegmentPositionContribution>& positions) {
  ...
  for (size_t i = 0; i < positions.size(); i++)
  {
    ...
    {
      if (positions[i].m_isFromAbove &&
          positions[i].m_lengthFromConnection < minDistFromContribAbove) {
      ...
      if (! positions[i].m_isFromAbove &&
          positions[i].m_lengthFromConnection < minDistFromContribBelow) {
  ...
  for (size_t i = 0; i < filteredPositions.size(); i++) {
    interpolatedValue +=
        (nominators[i]/denominator) * filteredPositions[i].m_connectionPosition;
  }
...
///
//------------------------------------------------------------------
void propagatePosContribDownwards(
  ...
  if ( posContribIt != segmentIdToPositionContrib.end()) {
    ...
    well_segment_type *segment = well_segment_collection_get(allErtSegments,
                                                             posContribIt->first);
    double sementLength = well_segment_get_length(segment);

    // If we do not have the contribution represented, add it, and accumulate
    // the length. If it is already present, do not touch.
...
//------------------------------------------------------------------
/// Helper class to determine whether a well connection is present
/// in a sub cell for a specific well. Connections must be tested
/// from innermost lgr to outermost since it accumulates the outer
/// cells having subcell connections as it goes.
//------------------------------------------------------------------
//class WellResultPointHasSubCellConnectionCalculator
//{
// public:
//  explicit WellResultPointHasSubCellConnectionCalculator(
//      const RigMainGrid* mainGrid,
//      well_state_type* ert_well_state): m_mainGrid(mainGrid) {
//
...
//          well_state_iget_wellhead(ert_well_state,
//                                   static_cast<int>(gridNr));
//
//      if ( ert_wellhead ) {
//        size_t localGridCellidx = localGridCellIndexFromErtConnection(
//            m_mainGrid->gridByIndex(gridNr), ert_wellhead, nullptr);
//        this->insertTheParentCells(gridNr, localGridCellidx);
//      }
//
//      std::string gridname =
//          gridNr == 0 ? ECL_GRID_GLOBAL_GRID: m_mainGrid->gridByIndex(gridNr)->gridName();
//
...
//
//            size_t localGridCellidx = localGridCellIndexFromErtConnection(
//                m_mainGrid->gridByIndex(gridNr), ert_connection, nullptr);
//            this->insertTheParentCells(gridNr, localGridCellidx);
//          }
//        }
//      }
...
//
//    size_t reservoirCellIdx =
//        m_mainGrid->reservoirCellIndexByGridAndGridLocalCellIndex(gridIndex,
//                                                                  gridCellIndex);
//
...
//      const RigCell& connectionCell =
//          m_mainGrid->cellByGridAndGridLocalCellIdx(gridIndex,
//                                                    gridCellIndex);
...
void RifReaderEclipseOutput::readWellCells(const ecl_grid_type* mainEclGrid,
  ...
  m_dynamicResultsAccess->readWellData(ert_well_info,
                                       importCompleteMswData);

  std::vector<double> daysSinceSimulationStart;
  std::vector<QDateTime> timeSteps;
  m_dynamicResultsAccess->timeSteps(&timeSteps,
...
//  caf::ProgressInfo progress(well_info_get_num_wells(ert_well_info), "");
  ...
  for (wellIdx = 0; wellIdx < well_info_get_num_wells(ert_well_info); wellIdx++) {
    ...
    well_ts_type* ert_well_time_series = well_info_get_ts(ert_well_info,
                                                          wellName);
    ...
    for (timeIdx = 0; timeIdx < timeStepCount; timeIdx++) {
      well_state_type* ert_well_state = well_ts_iget_state(ert_well_time_series,
                                                           timeIdx);
      ...
      if (importCompleteMswData && well_state_is_MSW(ert_well_state)) {
        ...
        // 1. Create separate visual branches for each Grid, with its own
        // wellhead
        // 2. Always use the connections to the grid with the highest number
        // (innermost LGR).
        // 3. Handle both and switch between them according to visual settings
        // of grid visualization
        //
        // Will there ever exist connections to different grids for the same
        // segment?
        ...
        for (int gridNr = lastGridNr; gridNr >= 0; --gridNr) {
          ...
          const well_conn_type* ert_wellhead = well_state_iget_wellhead(ert_well_state,
                                                                        static_cast<int>(gridNr));
          if (ert_wellhead) {
            wellResFrame.m_wellHead = createWellResultPoint(grids[gridNr],
                                                            ert_wellhead,
                                                            -1, -1, wellName);

            // HACK: Ert returns open as "this is equally wrong as closed for
            // well heads".
            // Well heads are not open jfr mail communication with HHGS and JH
            // Statoil 07.01.2016
        ...
        well_branch_collection_type* branches =
            well_state_get_branches(ert_well_state);

        int branchCount = well_branch_collection_get_size(branches);
        wellResFrame.m_wellResultBranches.resize( branchCount);

        std::map<int, std::vector<SegmentPositionContribution> >
            segmentIdToPositionContrib;
        std::vector<int> upperSegmentIdsOfUnpositionedSegementGroup;

        // For each branch, go from bottom segment upwards and transfer their
        // connections to WellResultpoints.
        // If they have no connections, create a resultpoint representing their
        // bottom position, which will receive an actual position at a later
        // stage.
        //
        // I addition, distribute contributions for calculating segment bottom
        // positions from bottom and up.
        ...
        for (int bIdx = 0; bIdx < well_branch_collection_get_size(branches); bIdx++) {
          ...
          while (segment && branchId == well_segment_get_branch_id(segment)) {
            ...
            for (int gridNr = lastGridNr; gridNr >= 0; --gridNr) {
              ...
              if (well_segment_has_grid_connections(segment, gridName.data())) {
                ...
                // Loop backwards to put the deepest connections first in the array.
                // (The segments are also traversed deep to shallow)
                for (int connIdx = connectionCount-1; connIdx >= 0; connIdx--) {
                  well_conn_type* ert_connection =
                      well_conn_collection_iget(connections, connIdx);
                  wellResultBranch.m_branchResultPoints.push_back(
                      createWellResultPoint(grids[gridNr], ert_connection,
                                            branchId, well_segment_get_id(segment),
                                            wellName));
                }
            ...
            // If the segment did not have connections at all, we need to
            // create a resultpoint representing the bottom of the segment
            ...
            if (!segmentHasConnections) {
              ...
              bool isAnInsolationContribution =
                  accLengthFromLastConnection < lastConnectionCellSize;
          ...
          // Add resultpoint representing the outlet segment (bottom),
          // if not the branch ends at the wellhead.
        ...
        // Propagate position contributions from connections
        // above unpositioned segments downwards
        well_segment_collection_type * allErtSegments =
            well_state_get_segments( ert_well_state );
            ...
            if ( resPoint.isCell() ) {
              ...
              if (previousResultPointWasCell) {
                ...
                SegmentPositionContribution posContrib(prevResPoint.m_ertSegmentId,
                                                       lastConnectionPos, 0.0, false,  -1,
                                                       prevResPoint.m_ertSegmentId, true);
        ...
        std::map<int, std::vector<SegmentPositionContribution> >::iterator
            posContribIt = segmentIdToPositionContrib.begin();
      ...
      else if ( false )
      {
        ...
        // Loop over all the grids in the model. If we have connections in one,
        // we will discard the main grid connections as the well connections are
        // duplicated in the main grid and LGR grids
        // Verified on 10 k case JJS. But smarter things could be done,
        // like showing the "main grid well" if turning off the LGR's.
        ...
        for (; gridNr < grids.size(); ++gridNr) {
          ...
          // Wellhead. If several grids have a wellhead definition for
          // this well, we use the last one. (Possibly the innermost LGR)
          const well_conn_type* ert_wellhead = well_state_iget_wellhead(
              ert_well_state, static_cast<int>(gridNr));
      ...
      {
        // Code handling None-MSW Wells ... Normal wells that is.
//        WellResultPointHasSubCellConnectionCalculator subCellConnCalc(
//            m_eclipseCase->mainGrid(), ert_well_state);
        int lastGridNr = static_cast<int>(grids.size()) - 1;
...
//------------------------------------------------------------------
QStringList
RifReaderEclipseOutput::validKeywordsForPorosityModel(const QStringList& keywords,
...
RifReaderEclipseOutput::createFilteredTimeStepInfos()
{
  std::vector<RigEclipseTimeStepInfo> timeStepInfos;
...
///
//------------------------------------------------------------------
void RifReaderEclipseOutput::extractResultValuesBasedOnPorosityModel(
    RiaDefines::PorosityModelType matrixOrFracture,
...
///
//------------------------------------------------------------------
//void RifReaderEclipseOutput::transferCoarseningInfo(
//    const ecl_grid_type* eclGrid, RigGridBase* grid)
//{
